...
from bitidea_constants import get_bitidea_home

# Load .env from ~/.bitidea/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from bitidea_cli.env_loader import load_bitidea_dotenv

_bitidea_home = get_bitidea_home()
_project_env = Path(__file__).parent / '.env'
_loaded_env_paths = load_bitidea_dotenv(bitidea_home=_bitidea_home, project_env=_project_env)
...
class _SafeWriter:
    ...
    When bitidea-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        try:
            from bitidea_cli.model_normalize import (
        ...
        # both live under ~/.bitidea/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from bitidea_logging import setup_logging, setup_verbose_logging
        setup_logging(bitidea_home=_bitidea_home)
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from bitidea_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        ...
                        "X-OpenRouter-Title": "Bitidea Agent",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif "api.githubcopilot.com" in effective_base.lower():
                    from bitidea_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        try:
                            from bitidea_cli.auth import PROVIDER_REGISTRY
                        ...
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `bitidea model`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `bitidea model` to "
                        "select a provider, or run `bitidea setup` for first-time "
                        "configuration."
                    )
        ...
        # Session logs go into ~/.bitidea/sessions/ alongside gateway sessions
        bitidea_home = get_bitidea_home()
        self.logs_dir = bitidea_home / "sessions"
        ...
        if self._session_db:
            try:
                self._session_db.create_session(
                    ...
                    source=self.platform or os.environ.get("BITIDEA_SESSION_SOURCE", "cli"),
                    model=self.model,
                    model_config={
        ...
        try:
            from bitidea_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if not _mem_provider_name:
                    try:
                        ...
                        if _hcfg.enabled and (_hcfg.api_key or _hcfg.base_url):
                            ...
                            try:
                                from bitidea_cli.config import load_config as _lc, save_config as _sc
                                _cfg = _lc()
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from bitidea_constants import get_bitidea_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                            "platform": platform or "cli",
                            "bitidea_home": str(_ghh()),
                        ...
                        try:
                            from bitidea_cli.profiles import get_active_profile_name
                            _profile = get_active_profile_name()
                            _init_kwargs["agent_identity"] = _profile
                            _init_kwargs["agent_workspace"] = "bitidea"
                        except Exception:
                            pass
        ...
        if _config_context_length is None:
            try:
                from bitidea_cli.config import get_compatible_custom_providers
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from bitidea_cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Bitidea Agent.  Choose a model with at least "
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            try:
                self.context_compressor.on_session_start(
                    self.session_id,
                    bitidea_home=str(get_bitidea_home()),
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        """
        import logging
        from bitidea_cli.providers import determine_api_mode
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``bitidea chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `bitidea setup` or set OPENROUTER_API_KEY."
                )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from bitidea_cli.models import _should_use_copilot_responses_api
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("BITIDEA_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the bitidea-agent install
        ...
        from bitidea_time import now as _bitidea_now
        now = _bitidea_now()
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from bitidea_cli.auth import resolve_codex_runtime_credentials
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from bitidea_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("BITIDEA_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("BITIDEA_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        _stale_base = float(os.getenv("BITIDEA_API_CALL_STALE_TIMEOUT", 300.0))
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            import httpx as _httpx
            _base_timeout = float(os.getenv("BITIDEA_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("BITIDEA_STREAM_READ_TIMEOUT", 120.0))
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode BITIDEA_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("BITIDEA_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("BITIDEA_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set BITIDEA_STREAM_STALE_TIMEOUT.
    ...
    def _try_activate_fallback(self) -> bool:
        ...
        try:
            ...
            try:
                from bitidea_cli.model_normalize import normalize_model_for_provider
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        api_kwargs = {
            ...
            "timeout": float(os.getenv("BITIDEA_API_TIMEOUT", 1800.0)),
        }
        if self._is_qwen_portal():
            api_kwargs["metadata"] = {
                "sessionId": self.session_id or "bitidea",
        ...
        if _is_nous:
            extra_body["tags"] = ["product=bitidea-agent"]
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from bitidea_cli.models import github_model_reasoning_efforts
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        try:
            from bitidea_cli.plugins import get_pre_tool_call_block_message
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=bitidea-agent"]
    ...
    def run_conversation(
        ...
        # ``bitidea logs --session <id>`` can filter a single conversation.
        from bitidea_logging import set_session_context
        ...
        if self._cached_system_prompt is None:
            ...
            if stored_prompt:
                ...
                try:
                    from bitidea_cli.plugins import invoke_hook as _invoke_hook
                    _invoke_hook(
                        "on_session_start",
                        session_id=self.session_id,
                        model=self.model,
        ...
        # are reused.  The system prompt is Bitidea's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for Bitidea internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if env_var_enabled("BITIDEA_DUMP_REQUESTS"):
                    ...
                    if (
                        ...
                        from bitidea_constants import display_bitidea_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Bitidea-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://console.anthropic.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: bitidea config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: bitidea config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `bitidea auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: bitidea setup", force=True)
