...
See: https://github.com/sabrinaberger083-sys/bitidea-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for bitidea_constants.get_subprocess_home()."""

    def test_returns_none_when_bitidea_home_unset(self, monkeypatch):
        monkeypatch.delenv("BITIDEA_HOME", raising=False)
        from bitidea_constants import get_subprocess_home
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        profile_home = bitidea_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        from bitidea_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".bitidea" / "profiles"
        ...
        monkeypatch.setenv("BITIDEA_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("BITIDEA_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(bitidea_home / "home")
    ...
    def test_no_injection_when_bitidea_home_unset(self, monkeypatch):
        monkeypatch.delenv("BITIDEA_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from bitidea_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from bitidea_cli.profiles import create_profile
