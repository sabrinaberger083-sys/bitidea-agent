"""Tests for acp_adapter.server — BitideaACPAgent ACP server."""
...
from acp_adapter.server import BitideaACPAgent, BITIDEA_VERSION
from acp_adapter.session import SessionManager
from bitidea_state import SessionDB
...
def agent(mock_manager):
    """BitideaACPAgent backed by a mock session manager."""
    return BitideaACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "bitidea-agent"
        assert resp.agent_info.version == BITIDEA_VERSION
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert BITIDEA_VERSION in result
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("bitidea_cli.config.load_config", lambda: {
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = BitideaACPAgent(session_manager=manager)
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["bitidea-acp"]
