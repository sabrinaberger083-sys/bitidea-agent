...
- Path resolution (absolute, relative to BITIDEA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp BITIDEA_HOME."""
    bitidea_home = tmp_path / ".bitidea"
    bitidea_home.mkdir()
    (bitidea_home / "cron").mkdir()
    (bitidea_home / "cron" / "output").mkdir()
    (bitidea_home / "scripts").mkdir()
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "BITIDEA_DIR", bitidea_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", bitidea_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", bitidea_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", bitidea_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("BITIDEA_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.bitidea/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        for key in (
            "BITIDEA_SESSION_PLATFORM",
            "BITIDEA_SESSION_CHAT_ID",
            "BITIDEA_SESSION_CHAT_NAME",
        ):
        ...
        assert os.environ.get("BITIDEA_SESSION_PLATFORM") is None
        assert os.environ.get("BITIDEA_SESSION_CHAT_ID") is None
        assert os.environ.get("BITIDEA_SESSION_CHAT_NAME") is None
