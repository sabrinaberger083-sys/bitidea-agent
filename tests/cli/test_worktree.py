...
def _setup_worktree(repo_root):
    ...
    wt_name = f"bitidea-{short_id}"
    branch_name = f"bitidea/{wt_name}"
...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("bitidea/bitidea-")
...
class TestStaleWorktreePruning:
    ...
    def test_prunes_old_clean_worktree(self, git_repo):
        ...
        for entry in worktrees_dir.iterdir():
            if not entry.is_dir() or not entry.name.startswith("bitidea-"):
                continue
            try:
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned bitidea/* and pr-* branches."""

    def test_prunes_orphaned_bitidea_branch(self, git_repo):
        """bitidea/bitidea-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "bitidea/bitidea-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "bitidea/bitidea-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "bitidea/bitidea-deadbeef" in result.stdout
        ...
        orphaned = [
            ...
            and (b.startswith("bitidea/bitidea-") or b.startswith("pr-"))
        ]
        assert "bitidea/bitidea-deadbeef" in orphaned
        ...
        assert "bitidea/bitidea-deadbeef" not in result.stdout
