...
def session_db(tmp_path):
    ...
    os.environ["BITIDEA_HOME"] = str(tmp_path / ".bitidea")
    os.makedirs(tmp_path / ".bitidea", exist_ok=True)
    from bitidea_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".bitidea" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal BitideaCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        BitideaCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        BitideaCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_fork_alias(self):
        ...
        from bitidea_cli.commands import resolve_command
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from bitidea_cli.commands import COMMAND_REGISTRY
