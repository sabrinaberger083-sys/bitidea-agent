...
def _isolate(tmp_path, monkeypatch):
    """Isolate BITIDEA_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal BitideaCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = BitideaCLI.__new__(BitideaCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["bitidea-core"]
        obj.compact = False
