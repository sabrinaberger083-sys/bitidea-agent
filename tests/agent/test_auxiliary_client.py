...
class TestReadCodexAccessToken:
    def test_valid_auth_store(self, tmp_path, monkeypatch):
        ...
        (bitidea_home / "auth.json").write_text(json.dumps({
            "version": 1,
            "providers": {
                "openai-codex": {
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("bitidea_cli.auth._read_codex_tokens", return_value={
...
class TestExpiredCodexFallback:
    ...
    def test_bitidea_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-bitidea-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
...
class TestExplicitProviderRouting:
    ...
    def test_explicit_google_alias_uses_gemini_credentials(self):
        ...
        with (
            patch("bitidea_cli.auth.resolve_api_key_provider_credentials", return_value={
                "api_key": "gemini-key",
...
class TestGetTextAuxiliaryClient:
    ...
    def test_custom_endpoint_over_codex(self, monkeypatch, codex_auth_dir):
        ...
        monkeypatch.setattr("bitidea_cli.config.load_config", lambda: config)
        monkeypatch.setattr("bitidea_cli.runtime_provider.load_config", lambda: config)
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("bitidea_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _try_codex
...
class TestTaskSpecificOverrides:
    ...
    def test_task_direct_endpoint_from_config(self, monkeypatch, tmp_path):
        bitidea_home = tmp_path / "bitidea"
        bitidea_home.mkdir(parents=True, exist_ok=True)
        (bitidea_home / "config.yaml").write_text(
            """auxiliary:
  web_extract:
...
def test_resolve_provider_client_supports_copilot_acp_external_process():
    ...
    with patch("agent.auxiliary_client._read_main_model", return_value="gpt-5.4-mini"), \
         ...
         patch("bitidea_cli.auth.resolve_external_process_provider_credentials", return_value={
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from bitidea_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("bitidea_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "bitidea_cli.auth.is_provider_explicitly_configured",
        lambda pid: False,
    )
