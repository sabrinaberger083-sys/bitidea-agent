...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        bitidea_home.mkdir()
        (bitidea_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"BITIDEA_HOME": str(bitidea_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(bitidea_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.bitidea/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        bitidea_home = tmp_path / ".bitidea"
        skills_dir = bitidea_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.bitidea/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        bitidea_home = tmp_path / ".bitidea"
        (bitidea_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"BITIDEA_HOME": str(bitidea_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.bitidea")

        assert mounts[0]["container_path"] == "/home/user/.bitidea/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.bitidea/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.bitidea/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape BITIDEA_HOME."""
        monkeypatch.setenv("BITIDEA_HOME", str(tmp_path / ".bitidea"))
        (tmp_path / ".bitidea").mkdir()

        # Create a sensitive file one level above bitidea_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside bitidea_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside BITIDEA_HOME must still be registered."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        (bitidea_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_bitidea_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of BITIDEA_HOME must be allowed."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        subdir = bitidea_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside BITIDEA_HOME pointing outside must be rejected."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))

        # Create a sensitive file outside bitidea_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside bitidea_home pointing outside
        symlink = bitidea_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes BITIDEA_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, bitidea_home: Path, cred_files: list):
        import yaml
        config_path = bitidea_home / "config.yaml"
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape BITIDEA_HOME."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        ...
        sensitive.write_text("{}")
        self._write_config(bitidea_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(bitidea_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside BITIDEA_HOME via config must still mount."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))

        (bitidea_home / "oauth.json").write_text("{}")
        self._write_config(bitidea_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / "cache" / "documents").mkdir(parents=True)
        (bitidea_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.bitidea/cache/documents" in paths
        assert "/root/.bitidea/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.bitidea/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        # Use legacy dir name — get_bitidea_dir prefers old if it exists
        (bitidea_home / "document_cache").mkdir()
        (bitidea_home / "image_cache").mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(bitidea_home / "document_cache") in host_paths
        assert str(bitidea_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.bitidea/cache/documents" in container_paths
        assert "/root/.bitidea/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        doc_dir = bitidea_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        ss_dir = bitidea_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.bitidea/cache/screenshots/session_abc/screen1.png"
