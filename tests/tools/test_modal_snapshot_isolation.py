...
def _restore_tool_modules():
    original_bitidea_home = os.environ.get("BITIDEA_HOME")
    original_modules = {
        name: module
        ...
        or name == "bitidea_cli"
        or name.startswith("bitidea_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_bitidea_home is None:
            os.environ.pop("BITIDEA_HOME", None)
        else:
            os.environ["BITIDEA_HOME"] = original_bitidea_home
        _reset_modules(("tools", "bitidea_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    bitidea_cli = types.ModuleType("bitidea_cli")
    bitidea_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["bitidea_cli"] = bitidea_cli
    bitidea_home = tmp_path / "bitidea-home"
    os.environ["BITIDEA_HOME"] = str(bitidea_home)
    sys.modules["bitidea_cli.config"] = types.SimpleNamespace(
        get_bitidea_home=lambda: bitidea_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="bitidea-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": bitidea_home / "modal_snapshots.json",
