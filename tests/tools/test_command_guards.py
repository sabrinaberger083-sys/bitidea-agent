...
def _clean_state():
    ...
    saved = {}
    for k in ("BITIDEA_INTERACTIVE", "BITIDEA_GATEWAY_SESSION", "BITIDEA_EXEC_ASK", "BITIDEA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithAllowSafeCommand:
    ...
    def test_both_allow(self, mock_tirith):
        os.environ["BITIDEA_INTERACTIVE"] = "1"
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["BITIDEA_INTERACTIVE"] = "1"
        session_key = os.getenv("BITIDEA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No BITIDEA_INTERACTIVE or BITIDEA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
