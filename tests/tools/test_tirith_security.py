...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._bitidea_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_bitidea_bin(self):
        """After _INSTALL_FAILED, manual install in BITIDEA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        bitidea_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(bitidea_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(bitidea_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._bitidea_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == bitidea_bin
            assert _tirith_mod._resolved_path == bitidea_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# BITIDEA_HOME isolation
# ---------------------------------------------------------------------------

class TestHermesHomeIsolation:
    def test_bitidea_bin_dir_respects_bitidea_home(self):
        """_bitidea_bin_dir must use BITIDEA_HOME, not hardcoded ~/.bitidea."""
        from tools.tirith_security import _bitidea_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"BITIDEA_HOME": tmpdir}):
            result = _bitidea_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_bitidea_home(self):
        """_failure_marker_path must use BITIDEA_HOME, not hardcoded ~/.bitidea."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"BITIDEA_HOME": "/custom/bitidea"}):
            result = _failure_marker_path()
        assert result == "/custom/bitidea/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets BITIDEA_HOME; verify it's active."""
        bitidea_home = os.getenv("BITIDEA_HOME")
        assert bitidea_home is not None, "BITIDEA_HOME should be set by conftest"
        assert "bitidea_test" in bitidea_home, "Should point to test temp dir"

    def test_get_bitidea_home_fallback(self):
        """Without BITIDEA_HOME set, falls back to ~/.bitidea."""
        from tools.tirith_security import _get_bitidea_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove BITIDEA_HOME entirely
            os.environ.pop("BITIDEA_HOME", None)
            result = _get_bitidea_home()
        assert result == os.path.join(os.path.expanduser("~"), ".bitidea")
