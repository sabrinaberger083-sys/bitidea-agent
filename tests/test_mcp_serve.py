"""
Tests for mcp_serve — Bitidea MCP server.
...
def _isolate_bitidea_home(tmp_path, monkeypatch):
    """Redirect BITIDEA_HOME to a temp directory."""
    monkeypatch.setenv("BITIDEA_HOME", str(tmp_path))
    try:
        import bitidea_constants
        monkeypatch.setattr(bitidea_constants, "get_bitidea_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking bitidea_state schema."""
