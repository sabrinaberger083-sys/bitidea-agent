...
and haven't been migrated to the Bitidea auth store yet.

Root cause: list_authenticated_providers() checked the raw Bitidea auth
...
Bitidea auth store has no openai-codex tokens, and
...
def codex_cli_only_env(tmp_path, monkeypatch):
    ...
    # Empty Bitidea auth store
    (bitidea_home / "auth.json").write_text(
...
def test_codex_cli_tokens_migrated_after_detection(codex_cli_only_env):
    ...
    migrated into the Bitidea auth store for subsequent fast lookups."""
    from bitidea_cli.model_switch import list_authenticated_providers

    # First call triggers migration
    list_authenticated_providers(current_provider="openai-codex")

    # Verify tokens are now in Bitidea auth store
    auth_path = codex_cli_only_env / "auth.json"
    store = json.loads(auth_path.read_text())
    providers = store.get("providers", {})
    assert "openai-codex" in providers, (
        f"openai-codex not migrated to Bitidea auth store: {list(providers.keys())}"
    )
...
@pytest.fixture()
def bitidea_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Bitidea auth store (no Codex CLI needed)."""
    bitidea_home = tmp_path / ".bitidea"
    bitidea_home.mkdir()
...
def test_normal_path_still_works(bitidea_auth_only_env):
    """openai-codex appears when tokens are already in Bitidea auth store."""
    from bitidea_cli.model_switch import list_authenticated_providers
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Bitidea
    auth store."""
    bitidea_home = tmp_path / ".bitidea"
    bitidea_home.mkdir()
