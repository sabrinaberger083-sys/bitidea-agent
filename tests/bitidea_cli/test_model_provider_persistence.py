"""Tests that provider selection via `bitidea model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated BITIDEA_HOME with a minimal string-format config."""
    ...
    monkeypatch.delenv("BITIDEA_MODEL", raising=False)
    monkeypatch.delenv("LLM_MODEL", raising=False)
    monkeypatch.delenv("BITIDEA_INFERENCE_PROVIDER", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from bitidea_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from bitidea_cli.main import _model_flow_api_key_provider
        ...
        with patch("bitidea_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("bitidea_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from bitidea_cli.main import _model_flow_copilot
        from bitidea_cli.config import load_config

        with patch(
            "bitidea_cli.auth.resolve_api_key_provider_credentials",
            return_value={
                "provider": "copilot",
            ...
            "bitidea_cli.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "bitidea_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "bitidea_cli.auth.deactivate_provider",
        ):
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from bitidea_cli.main import _model_flow_copilot_acp
        from bitidea_cli.config import load_config

        with patch(
            "bitidea_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "bitidea_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("bitidea_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("bitidea_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("bitidea_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("bitidea_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("bitidea_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("bitidea_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        from bitidea_cli.main import _model_flow_api_key_provider
        from bitidea_cli.config import load_config, get_env_value

        # User types a shell command instead of a URL at the base URL prompt
        with patch("bitidea_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("bitidea_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.bitidea/.env"):
