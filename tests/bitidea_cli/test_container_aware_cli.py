...
def container_env(tmp_path, monkeypatch):
    """Set up a fake BITIDEA_HOME with .container-mode file."""
    bitidea_home = tmp_path / ".bitidea"
    bitidea_home.mkdir()
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
    monkeypatch.delenv("BITIDEA_DEV", raising=False)

    container_mode = bitidea_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=bitidea-agent\n"
        "exec_user=bitidea\n"
        "bitidea_bin=/data/current-package/bin/bitidea\n"
    )
    return bitidea_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("bitidea_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "bitidea-agent"
    assert info["exec_user"] == "bitidea"
    assert info["bitidea_bin"] == "/data/current-package/bin/bitidea"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("bitidea_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_bitidea_dev(container_env, monkeypatch):
    """Returns None when BITIDEA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("BITIDEA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_bitidea_dev_zero(container_env, monkeypatch):
    """BITIDEA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("BITIDEA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        bitidea_home = Path(tmpdir) / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("bitidea_constants.is_container", return_value=False), \
             patch("bitidea_cli.config.get_bitidea_home", return_value=bitidea_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BITIDEA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=bitidea-custom\n"
        "exec_user=myuser\n"
        "bitidea_bin=/opt/bitidea/bin/bitidea\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "bitidea-custom"
    assert info["exec_user"] == "myuser"
    assert info["bitidea_bin"] == "/opt/bitidea/bin/bitidea"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from bitidea_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "bitidea"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "bitidea-agent" in cmd
    assert "/data/current-package/bin/bitidea" in cmd
    assert "chat" in cmd
