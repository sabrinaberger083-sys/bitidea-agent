...
import bitidea_cli.doctor as doctor
import bitidea_cli.gateway as gateway_cli
from bitidea_cli import doctor as doctor_mod
from bitidea_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "BITIDEA_HOME", bitidea_home)
    monkeypatch.delenv("BITIDEA_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("BITIDEA_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_bitidea_home(self, tmp_path, provider=""):
        """Create a minimal BITIDEA_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_bitidea_home(tmp_path, provider)
