"""Tests for the Command Installation check in bitidea doctor."""
...
import bitidea_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal BITIDEA_HOME + PROJECT_ROOT for doctor tests."""
    ...
    bitidea_bin = venv_bin_dir / "bitidea"
    bitidea_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    bitidea_bin.chmod(0o755)

    monkeypatch.setattr(doctor_mod, "BITIDEA_HOME", home)
    ...
    # Stub auth checks
    try:
        from bitidea_cli import auth as _auth_mod
    ...
    return home, project, bitidea_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, bitidea_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "bitidea"
        cmd_link.symlink_to(bitidea_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "bitidea doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "bitidea"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == bitidea_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "bitidea"
        wrong_target = tmp_path / "wrong_bitidea"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        bitidea_bin = project / ".venv" / "bin" / "bitidea"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "bitidea"
        cmd_link.symlink_to(bitidea_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/bitidea" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/bitidea is a regular file (not symlink), accept it."""
        home, project, bitidea_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "bitidea"
        cmd_link.write_text("#!/bin/sh\nexec python -m bitidea_cli.main \"$@\"\n")
