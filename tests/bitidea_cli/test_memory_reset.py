"""Tests for the `bitidea memory reset` CLI command.
...
- Profile-scoped reset (uses BITIDEA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake BITIDEA_HOME with memory files."""
    bitidea_home = tmp_path / ".bitidea"
    memories = bitidea_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.bitidea/bitidea-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `bitidea memory reset` is run.
    """
    from bitidea_constants import get_bitidea_home, display_bitidea_home

    mem_dir = get_bitidea_home() / "memories"
    files_to_reset = []
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        (bitidea_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's BITIDEA_HOME."""
        ...
        monkeypatch.setenv("BITIDEA_HOME", str(profile_home))
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))

        # The memories dir won't exist; get_bitidea_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
