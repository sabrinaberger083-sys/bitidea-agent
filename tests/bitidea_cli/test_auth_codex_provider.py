"""Tests for Codex auth — tokens stored in Bitidea auth store (~/.bitidea/auth.json)."""
...
def _setup_bitidea_auth(bitidea_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Bitidea auth store."""
    bitidea_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    auth_file = bitidea_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_bitidea_auth(bitidea_home)
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    bitidea_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (bitidea_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    bitidea_home = tmp_path / "bitidea"
    _setup_bitidea_auth(bitidea_home, access_token="")
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_bitidea_auth(bitidea_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
    ...
    monkeypatch.setattr("bitidea_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    bitidea_home = tmp_path / "bitidea"
    _setup_bitidea_auth(bitidea_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Bitidea auth store, not ~/.codex/."""
    bitidea_home = tmp_path / "bitidea"
    codex_home = tmp_path / "codex-cli"
    bitidea_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "bitidea-at", "refresh_token": "bitidea-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Bitidea store
    assert not (codex_home / "auth.json").exists()

    # Bitidea auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "bitidea-at"
...
def test_refresh_codex_auth_tokens_writes_back_to_cli(tmp_path, monkeypatch):
    ...
    from bitidea_cli.auth import _refresh_codex_auth_tokens
    ...
    monkeypatch.setattr("bitidea_cli.auth.refresh_codex_oauth_pure", lambda *a, **kw: {
...
def test_resolve_returns_bitidea_auth_store_source(tmp_path, monkeypatch):
    bitidea_home = tmp_path / "bitidea"
    _setup_bitidea_auth(bitidea_home)
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "bitidea-auth-store"
