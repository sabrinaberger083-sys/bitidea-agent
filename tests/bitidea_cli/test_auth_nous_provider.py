...
from bitidea_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from bitidea_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("BITIDEA_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_bitidea_ca_bundle_env_falls_back(self, monkeypatch):
        from bitidea_cli.auth import _resolve_verify

        monkeypatch.setenv("BITIDEA_CA_BUNDLE", "/nonexistent/bitidea-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (bitidea_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from bitidea_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    bitidea_home = tmp_path / "bitidea"
    _setup_nous_auth(bitidea_home, access_token="at-123")
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    bitidea_home = tmp_path / "bitidea"
    _setup_nous_auth(bitidea_home, refresh_token="refresh-old")
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
    ...
    monkeypatch.setattr("bitidea_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("bitidea_cli.auth._mint_agent_key", _fake_mint_agent_key)
