"""Tests for the Bitidea plugin system (bitidea_cli.plugins)."""
...
from bitidea_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.bitidea/plugins/ are discovered."""
        plugins_dir = tmp_path / "bitidea_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("BITIDEA_HOME", str(tmp_path / "bitidea_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("BITIDEA_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".bitidea" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under bitidea_plugins.<name>."""
        plugins_dir = tmp_path / "bitidea_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("BITIDEA_HOME", str(tmp_path / "bitidea_test"))

        # Clean up any prior namespace module
        sys.modules.pop("bitidea_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "bitidea_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="bitidea_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "bitidea_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import bitidea_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("bitidea_cli.plugins._plugin_manager", mgr):
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("bitidea_cli.plugins.PluginContext.dispatch_tool.__module__", "bitidea_cli.plugins"):
