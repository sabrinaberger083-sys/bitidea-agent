...
subparser, placing the flag BEFORE the subcommand (e.g. 'bitidea --yolo chat')
...
def _build_parser():
    """Build the bitidea argument parser from the real code.
    ...
    """
    parser = argparse.ArgumentParser(prog="bitidea")
...
class TestYoloEnvVar:
    """Verify --yolo sets BITIDEA_YOLO_MODE regardless of flag position.
    ...
    def _clean_env(self):
        os.environ.pop("BITIDEA_YOLO_MODE", None)
        yield
        os.environ.pop("BITIDEA_YOLO_MODE", None)
    ...
    def test_no_yolo_no_env(self):
        ...
        args = parser.parse_args(["chat"])
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("BITIDEA_YOLO_MODE") is None
