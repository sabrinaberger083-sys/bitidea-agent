"""Tests for bitidea backup and import commands."""
...
def _make_bitidea_tree(root: Path) -> None:
    """Create a realistic ~/.bitidea directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "bitidea_state.db").write_bytes(b"fake-state")
    ...
    # bitidea-agent repo (should be EXCLUDED)
    (root / "bitidea-agent").mkdir(exist_ok=True)
    (root / "bitidea-agent" / "run_agent.py").write_text("# big file\n")
    (root / "bitidea-agent" / ".git").mkdir()
    (root / "bitidea-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_bitidea_agent(self):
        from bitidea_cli.backup import _should_exclude
        assert _should_exclude(Path("bitidea-agent/run_agent.py"))
        assert _should_exclude(Path("bitidea-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        _make_bitidea_tree(bitidea_home)

        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        # get_default_bitidea_root needs this
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        out_zip = tmp_path / "backup.zip"
        args = Namespace(output=str(out_zip))

        from bitidea_cli.backup import run_backup
        run_backup(args)
    ...
    def test_excludes_bitidea_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include bitidea-agent/ directory."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        _make_bitidea_tree(bitidea_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "bitidea-agent" in n]
            assert agent_files == [], f"bitidea-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/bitidea-backup-*.zip."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("bitidea-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Bitidea backup."""
        from bitidea_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only bitidea_state.db (old wrong name) is rejected."""
        from bitidea_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["bitidea_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into bitidea home."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        from bitidea_cli.backup import run_import
        run_import(args)

        assert (bitidea_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (bitidea_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (bitidea_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (bitidea_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_bitidea_prefix(self, tmp_path, monkeypatch):
        """Import strips .bitidea/ prefix if all entries share it."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".bitidea/config.yaml": "model: test\n",
            ".bitidea/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (bitidea_home / "config.yaml").read_text() == "model: test\n"
        assert (bitidea_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_bitidea_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a bitidea backup."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (bitidea_home / "config.yaml").exists()
        # traversal file should NOT exist outside bitidea home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (bitidea_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (bitidea_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (bitidea_home / "config.yaml").read_text() == "model: restored\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".bitidea"
        src_home.mkdir(parents=True)
        _make_bitidea_tree(src_home)

        monkeypatch.setenv("BITIDEA_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from bitidea_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".bitidea"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("BITIDEA_HOME", str(dst_home))
        ...
        # bitidea-agent should NOT be present
        assert not (dst_home / "bitidea-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from bitidea_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without bitidea markers fails validation."""
        import io
        from bitidea_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_bitidea(self):
        """Detects .bitidea/ prefix wrapping all entries."""
        import io
        from bitidea_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".bitidea/config.yaml", "test")
            zf.writestr(".bitidea/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".bitidea/"
...
class TestBackupEdgeCases:
    def test_nonexistent_bitidea_home(self, tmp_path, monkeypatch):
        """Backup exits when bitidea home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".bitidea"
        monkeypatch.setenv("BITIDEA_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("bitidea-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_bitidea_home(self, tmp_path, monkeypatch):
        """Backup handles empty bitidea home (no files to back up)."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        # Only excluded dirs, no actual files
        (bitidea_home / "__pycache__").mkdir()
        (bitidea_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = bitidea_home / "secret.db"
    ...
    def test_skips_output_zip_inside_bitidea(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside bitidea root."""
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside bitidea home
        out_zip = bitidea_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir()
        (bitidea_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = bitidea_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (bitidea_home / "config.yaml").exists()
        assert (bitidea_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (bitidea_home / "profiles" / "coder" / "config.yaml").exists()
        assert (bitidea_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "bitidea -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import bitidea_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "bitidea_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from bitidea_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def bitidea_home(self, tmp_path):
        """Create a fake BITIDEA_HOME with critical state files."""
        home = tmp_path / ".bitidea"
        home.mkdir()
    ...
    def test_creates_snapshot(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(bitidea_home=bitidea_home)
        assert snap_id is not None
        snap_dir = bitidea_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", bitidea_home=bitidea_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(bitidea_home=bitidea_home)
        db_copy = bitidea_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(bitidea_home=bitidea_home)
        assert (bitidea_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(bitidea_home=bitidea_home)
        with open(bitidea_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(bitidea_home=empty) is None

    def test_list_snapshots(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", bitidea_home=bitidea_home)
        id2 = create_quick_snapshot(label="second", bitidea_home=bitidea_home)

        snaps = list_quick_snapshots(bitidea_home=bitidea_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", bitidea_home=bitidea_home)
        snaps = list_quick_snapshots(limit=3, bitidea_home=bitidea_home)
        assert len(snaps) == 3

    def test_restore_config(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(bitidea_home=bitidea_home)

        (bitidea_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (bitidea_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, bitidea_home=bitidea_home)
        assert result is True
        assert "openrouter" in (bitidea_home / "config.yaml").read_text()

    def test_restore_state_db(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(bitidea_home=bitidea_home)

        conn = sqlite3.connect(str(bitidea_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, bitidea_home=bitidea_home)
    ...
    def test_restore_nonexistent(self, bitidea_home):
        from bitidea_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", bitidea_home=bitidea_home) is False

    def test_auto_prune(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", bitidea_home=bitidea_home)
        snaps = list_quick_snapshots(limit=100, bitidea_home=bitidea_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, bitidea_home):
        from bitidea_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", bitidea_home=bitidea_home)
        deleted = prune_quick_snapshots(keep=3, bitidea_home=bitidea_home)
        assert deleted == 7
        assert len(list_quick_snapshots(bitidea_home=bitidea_home)) == 3
