"""Tests for the top-level `./bitidea` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./bitidea` should use `bitidea_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "bitidea"
    called = []

    fake_main_module = types.ModuleType("bitidea_cli.main")

    def fake_main():
        called.append("bitidea_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "bitidea_cli.main", fake_main_module)
    ...
    assert called == ["bitidea_cli.main"]
