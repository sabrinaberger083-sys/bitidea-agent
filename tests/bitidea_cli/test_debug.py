"""Tests for ``bitidea debug`` CLI command and debug utilities."""
...
def bitidea_home(tmp_path, monkeypatch):
    """Set up an isolated BITIDEA_HOME with minimal logs."""
    home = tmp_path / ".bitidea"
    home.mkdir()
    monkeypatch.setenv("BITIDEA_HOME", str(home))
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from bitidea_cli.debug import _upload_paste_rs
        ...
        with patch("bitidea_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "bitidea_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from bitidea_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from bitidea_cli.debug import upload_to_pastebin

        with patch("bitidea_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("bitidea_cli.debug._upload_paste_rs",
             ...
             patch("bitidea_cli.debug._upload_dpaste_com",
...
class TestReadFullLog:
    ...
    def test_reads_small_file(self, bitidea_home):
        from bitidea_cli.debug import _read_full_log
    ...
    def test_returns_none_for_empty(self, bitidea_home):
        # Truncate agent.log to empty
        (bitidea_home / "logs" / "agent.log").write_text("")

        from bitidea_cli.debug import _read_full_log
        assert _read_full_log("agent") is None

    def test_truncates_large_file(self, bitidea_home):
        """Files larger than max_bytes get tail-truncated."""
        from bitidea_cli.debug import _read_full_log

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (bitidea_home / "logs" / "agent.log").write_text(big_content * 200)

        content = _read_full_log("agent", max_bytes=1024)
        assert content is not None
        assert "truncated" in content

    def test_unknown_log_returns_none(self, bitidea_home):
        from bitidea_cli.debug import _read_full_log
        assert _read_full_log("nonexistent") is None

    def test_falls_back_to_rotated_file(self, bitidea_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from bitidea_cli.debug import _read_full_log

        logs_dir = bitidea_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, bitidea_home):
    ...
    def test_falls_back_when_primary_empty(self, bitidea_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, bitidea_home):
        from bitidea_cli.debug import collect_debug_report

        with patch("bitidea_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- bitidea dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- bitidea dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, bitidea_home):
        from bitidea_cli.debug import collect_debug_report

        with patch("bitidea_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, bitidea_home):
        from bitidea_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, bitidea_home):
        from bitidea_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_local_flag_prints_full_logs(self, bitidea_home, capsys):
        """--local prints the report plus full log contents."""
        from bitidea_cli.debug import run_debug_share
    ...
    def test_share_uploads_three_pastes(self, bitidea_home, capsys):
        ...
        with patch("bitidea_cli.dump.run_dump") as mock_dump, \
             patch("bitidea_cli.debug.upload_to_pastebin",
                    side_effect=_mock_upload):
            mock_dump.side_effect = lambda a: print("--- bitidea dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- bitidea dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- bitidea dump ---" in gateway_paste
    ...
    def test_share_continues_on_log_upload_failure(self, bitidea_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, bitidea_home, capsys):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        from bitidea_cli.debug import run_debug
        ...
        out = capsys.readouterr().out
        assert "bitidea debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, bitidea_home):
        from bitidea_cli.debug import run_debug
...
class TestArgparseIntegration:
    def test_module_imports_clean(self):
        from bitidea_cli.debug import run_debug, run_debug_share
        assert callable(run_debug)
        assert callable(run_debug_share)

    def test_cmd_debug_dispatches(self):
        from bitidea_cli.main import cmd_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from bitidea_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from bitidea_cli.debug import delete_paste
        ...
        with patch("bitidea_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    def test_spawns_detached_process(self):
        from bitidea_cli.debug import _schedule_auto_delete
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from bitidea_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("bitidea_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("bitidea_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, bitidea_home, capsys):
        from bitidea_cli.debug import run_debug_share
        ...
        with patch("bitidea_cli.dump.run_dump"), \
             ...
             patch("bitidea_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, bitidea_home, capsys):
        from bitidea_cli.debug import run_debug_share
        ...
        with patch("bitidea_cli.dump.run_dump"), \
             ...
             patch("bitidea_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, bitidea_home, capsys):
        from bitidea_cli.debug import run_debug_share
