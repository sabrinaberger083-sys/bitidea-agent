...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure BITIDEA_HOME and RETAINDB vars are isolated."""
...
class TestClient:
    ...
    def test_get_agent_model_path(self):
        ...
        with patch.object(c, "request") as mock_req:
            ...
            c.get_agent_model("bitidea")
            mock_req.assert_called_once_with(
                "GET", "/v1/memory/agent/bitidea/model",
...
class TestRetainDBMemoryProvider:
    ...
    def _make_provider(self, tmp_path, monkeypatch, api_key="rdb-test-key"):
        ...
        (tmp_path / ".bitidea").mkdir(exist_ok=True)
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", bitidea_home=str(tmp_path / ".bitidea"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", bitidea_home=profile_home)
        assert p._client.project == "bitidea-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".bitidea" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        bitidea_home = tmp_path / ".bitidea"
        bitidea_home.mkdir(exist_ok=True)
        monkeypatch.setenv("BITIDEA_HOME", str(bitidea_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", bitidea_home=str(bitidea_home))
        return p
