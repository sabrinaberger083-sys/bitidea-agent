"""
Tests for timezone support (bitidea_time module + integration points).
...
def _reset_bitidea_time_cache():
    """Reset the bitidea_time module cache (replacement for removed reset_cache)."""
    bitidea_time._cached_tz = None
    bitidea_time._cached_tz_name = None
    bitidea_time._cache_resolved = False


# =========================================================================
# bitidea_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def setup_method(self):
        _reset_bitidea_time_cache()

    def teardown_method(self):
        _reset_bitidea_time_cache()
        os.environ.pop("BITIDEA_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["BITIDEA_TIMEZONE"] = "Asia/Kolkata"
        result = bitidea_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["BITIDEA_TIMEZONE"] = "UTC"
        result = bitidea_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["BITIDEA_TIMEZONE"] = "America/New_York"
        result = bitidea_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["BITIDEA_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="bitidea_time"):
            result = bitidea_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["BITIDEA_TIMEZONE"] = "Europe/London"
        tz = bitidea_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["BITIDEA_TIMEZONE"] = "Not/A/Timezone"
        tz = bitidea_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When BITIDEA_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["BITIDEA_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When BITIDEA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("BITIDEA_TIMEZONE", None)
    ...
    def test_bitidea_timezone_not_leaked_to_child(self):
        """BITIDEA_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["BITIDEA_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("BITIDEA_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=bitidea_tz) which shifted
        absolute time when system-local tz != Bitidea tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_bitidea_tz(self):
        """Already-aware datetimes should be normalized to Bitidea tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Bitidea tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Bitidea tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Bitidea is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Bitidea tz
        ...
        # Use a Bitidea timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _bitidea_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["BITIDEA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_bitidea_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Bitidea timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["BITIDEA_TIMEZONE"] = "US/Eastern"
        _reset_bitidea_time_cache()
