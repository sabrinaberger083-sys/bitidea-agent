...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_bitidea_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("bitidea_constants.display_bitidea_home", return_value="~/.bitidea/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.bitidea/profiles/coder/cron/output/" in prompt
