...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="BitideaBot"),
    )
...
async def test_handle_thread_create_slash_dispatches_session_when_message_provided(adapter):
    ...
    await adapter._handle_thread_create_slash(interaction, "Planning", "Hello Bitidea", 1440)

    adapter._dispatch_thread_session.assert_awaited_once_with(
        interaction, "555", "Planning", "Hello Bitidea",
    )
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    bitidea_dir = tmp_path / ".bitidea"
    bitidea_dir.mkdir()
    config_path = bitidea_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("BITIDEA_HOME", str(bitidea_dir))
...
def test_register_skill_group_creates_group(adapter):
    ...
    with patch(
        "bitidea_cli.commands.discord_skill_commands_by_category",
