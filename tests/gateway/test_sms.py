...
class TestSmsToolset:
    def test_bitidea_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("bitidea-sms")
        assert ts is not None
        assert "tools" in ts

    def test_bitidea_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("bitidea-gateway")
        assert gw is not None
        assert "bitidea-sms" in gw["includes"]
